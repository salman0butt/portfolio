...
function renderInline(text: string): ReactNode[] {
  ...
  return parts.map((part, index) => {
    ...
    if (part.startsWith('`') && part.endsWith('`')) return <code key={index} className="rounded bg-gray-100 px-1.5 py-0.5 text-[0.9em] text-emerald-700 dark:bg-gray-800 dark:text-emerald-300">{part.slice(1, -1)}</code>;

    const linkMatch = part.match(/^\[([^\]]+)\]\(([^)]+)\)$/);
    if (linkMatch) {
      const href = safeHref(linkMatch[2]);
      const external = href.startsWith('http');
      return <a key={index} href={href} {...(external ? { target: '_blank', rel: 'noreferrer noopener' } : {})} className="font-medium text-emerald-600 underline decoration-emerald-500/30 underline-offset-4 hover:decoration-emerald-500 dark:text-emerald-400">{linkMatch[1]}</a>;
    }
    return part;
  });
}
...
export default function MarkdownContent({ content }: { content: string }) {
  ...
  while (index < lines.length) {
    ...
    if (/^>\s/.test(line)) {
      ...
      blocks.push(<blockquote key={`quote-${index}`} className="my-7 border-l-4 border-emerald-500 bg-emerald-50/70 px-5 py-4 italic text-gray-700 dark:bg-emerald-500/5 dark:text-gray-300">{renderInline(quote.join(' '))}</blockquote>);
      continue;
    }
    ...
    index += 1;
    while (index < lines.length && lines[index].trim() && !isSpecialLine(lines[index])) { paragraph.push(lines[index].trim()); index += 1; }
